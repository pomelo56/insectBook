...
function generateInsectExternalId() {
  return 'insect_' + Date.now().toString(36) + Math.random().toString(36).substring(2, 9);
}
  ...
  try {
    ...
    for (const [insectId, insectInfo] of insectMap.entries()) {
      ...
      if (existingInsectIds.has(insectId)) {
        // 更新现有昆虫
        try {
          // 检查是否已有externalId，如果没有则添加
          const insectDoc = await db.collection('insects').doc(insectId).get();
          if (!insectDoc.data.externalId) {
            insectData.externalId = generateInsectExternalId();
          }
        ...
        try {
          await db.collection('insects').add({
            data: {
              ...
              externalId: generateInsectExternalId()
            }
          });
          addedCount++;
